...
namespace Controladores
{
    public class OfertaControlador : IControlador<Oferta>
    {
        ...
        public async Task<Oferta> add(Oferta ofertaNueva)
        {
            ...
            HttpResponseMessage mensaje = await cliente.PostAsync("http://10.182.169.200:8080/api/ofertas", content);
        ...
        public async Task<List<Oferta>> getAll()
        {
            try
            {
                ...
                HttpResponseMessage mensaje = await cliente.GetAsync("http://10.182.169.200:8080/api/ofertas");
        ...
        {
            Oferta oferta = new Oferta();
            HttpResponseMessage mensaje = await cliente.GetAsync($"http://10.182.169.200:8080/api/ofertas/{id}");
            mensaje.EnsureSuccessStatusCode();
            string mensajeJson = await mensaje.Content.ReadAsStringAsync();
        ...
        {
            HttpResponseMessage mensaje = await cliente.DeleteAsync($"http://10.182.169.200:8080/api/ofertas/{id}");
            mensaje.EnsureSuccessStatusCode();
            return true;
        }
        ...
        public async Task<Oferta> update(Oferta ofertaModificada, int id)
        {
            ...
            HttpResponseMessage mensaje = await cliente.PutAsync($"http://10.182.169.200:8080/api/ofertas/{id}", content);
            mensaje.EnsureSuccessStatusCode();
