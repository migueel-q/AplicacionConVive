...
namespace Controladores
{
    public class ContratoControlador : IControlador<Contrato>
    {
        ...
        public async Task<Contrato> add(Contrato contratoNuevo)
        {
            ...
            HttpResponseMessage mensaje = await cliente.PostAsync("http://10.182.169.200:8080/api/contratos", content);
        ...
        public async Task<List<Contrato>> getAll()
        {
            try
            {
                ...
                HttpResponseMessage mensaje = await cliente.GetAsync("http://10.182.169.200:8080/api/contratos");
        ...
        {
            Contrato contrato = new Contrato();
            HttpResponseMessage mensaje = await cliente.GetAsync($"http://10.182.169.200:8080/api/contratos/{id}");
            mensaje.EnsureSuccessStatusCode();
            string mensajeJson = await mensaje.Content.ReadAsStringAsync();
        ...
        {
            HttpResponseMessage mensaje = await cliente.DeleteAsync($"http://10.182.169.200:8080/api/contratos/{id}");
            mensaje.EnsureSuccessStatusCode();
            return true;
        }
        ...
        public async Task<Contrato> update(Contrato ofertaModificada, int id)
        {
            ...
            HttpResponseMessage mensaje = await cliente.PutAsync($"http://10.182.169.200:8080/api/contratos/{id}", content);
            mensaje.EnsureSuccessStatusCode();
