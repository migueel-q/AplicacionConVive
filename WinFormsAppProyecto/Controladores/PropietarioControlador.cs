...
namespace Controladores
{
    public class PropietarioControlador : IControlador<Propietario>
    {
        ...
        public async Task<Propietario> add(Propietario propietarioNuevo)
        {
            ...
            HttpResponseMessage mensaje = await cliente.PostAsync("http://10.182.169.200:8080/api/propietarios", content);
        ...
        public async Task<List<Propietario>> getAll()
        {
            ...
            HttpResponseMessage mensaje = await cliente.GetAsync("http://10.182.169.200:8080/api/propietarios");
        ...
        {
            Propietario propietario = new Propietario();
            HttpResponseMessage mensaje = await cliente.GetAsync($"http://10.182.169.200:8080/api/propietarios/{id}");
            mensaje.EnsureSuccessStatusCode();
            string mensajeJson = await mensaje.Content.ReadAsStringAsync();
        ...
        {
            HttpResponseMessage mensaje = await cliente.DeleteAsync($"http://10.182.169.200:8080/api/propietarios/{id}");
            mensaje.EnsureSuccessStatusCode();
            return true;
        }
        ...
        public async Task<Propietario> update(Propietario propietarioModificado, int id)
        {
            ...
            HttpResponseMessage mensaje = await cliente.PutAsync($"http://10.182.169.200:8080/api/propietarios/{id}", content);
            mensaje.EnsureSuccessStatusCode();
