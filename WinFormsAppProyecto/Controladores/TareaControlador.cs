...
namespace Controladores
{
    public class TareaControlador : IControlador<Tarea>
    {
        ...
        public async Task<Tarea> add(Tarea tareaNueva)
        {
            ...
            HttpResponseMessage mensaje = await cliente.PostAsync("http://10.182.169.200:8080/api/tareas", content);
        ...
        public async Task<List<Tarea>> getAll()
        {
            try
            {
                ...
                HttpResponseMessage mensaje = await cliente.GetAsync("http://10.182.169.200:8080/api/tareas");
        ...
        {
            Tarea tarea = new Tarea();
            HttpResponseMessage mensaje = await cliente.GetAsync($"http://10.182.169.200:8080/api/tareas/{id}");
            mensaje.EnsureSuccessStatusCode();
            string mensajeJson = await mensaje.Content.ReadAsStringAsync();
        ...
        {
            HttpResponseMessage mensaje = await cliente.DeleteAsync($"http://10.182.169.200:8080/api/tareas/{id}");
            mensaje.EnsureSuccessStatusCode();
            return true;
        }
        ...
        public async Task<Tarea> update(Tarea tareaModificada, int id)
        {
            ...
            HttpResponseMessage mensaje = await cliente.PutAsync($"http://10.182.169.200:8080/api/tareas/{id}", content);
            mensaje.EnsureSuccessStatusCode();
